"""
Seedstr API Client for BlitzDev
Based on official Seedstr API: https://github.com/seedstr/seed-agent
Handles job polling, file uploads (base64), and response submission
"""
...
class SeedstrError(Exception):
    """Base exception for Seedstr API errors"""
    pass


class SeedstrAuthError(SeedstrError):
    """Authentication error"""
    pass


class SeedstrTimeoutError(SeedstrError):
    """Request timeout"""
    pass
...
class SeedstrClient:
    ...
    def __init__(
        ...
        self.api_url = (api_url or settings.SEEDSTR_API_URL).rstrip('/')
        self.api_url_v2 = (api_url_v2 or settings.SEEDSTR_API_URL_V2 or self.api_url).rstrip('/')
        self.api_key = api_key or settings.SEEDSTR_API_KEY
        self.poll_interval = poll_interval or settings.SEEDSTR_POLL_INTERVAL
        self.timeout = timeout or settings.SEEDSTR_TIMEOUT
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self.session is None or self.session.closed:
            headers = {
                ...
                "User-Agent": f"BlitzDev/{settings.APP_VERSION}"
            }
    ...
    async def upload_file(self, file_path: Path) -> FileAttachment:
        """
        Upload a file to Seedstr (base64 encoded)
        ...
        if not file_path.exists():
            raise SeedstrError(f"File not found: {file_path}")
        ...
        if not result.get("success"):
            raise SeedstrError("Upload failed: No success flag")
        
        files = result.get("files", [])
        if not files:
            raise SeedstrError("Upload failed: No files returned")
...
# Singleton instance
_client: Optional[SeedstrClient] = None


def get_seedstr_client() -> SeedstrClient:
    """Get or create Seedstr client singleton"""
    global _client
    if _client is None:
        _client = SeedstrClient()
    return _client
